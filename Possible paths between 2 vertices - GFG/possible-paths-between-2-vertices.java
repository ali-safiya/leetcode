...
class Solution {
    ...
    public int countPaths(int V, ArrayList<ArrayList<Integer>> adj, int source,
        ...
        count += DFS(adj,visited,source,destination);
    ...
    int DFS(ArrayList<ArrayList<Integer>> adj , boolean[] visited , int src, int dist){
        
        
        if(src==dist){
            // System.out.println("bskan");
            return 1;
        }
        ...
        for(int child : adj.get(src)){
            ...
            if(!visited[child]){
                count += DFS(adj,visited,child,dist);
